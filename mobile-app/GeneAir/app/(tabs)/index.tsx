...
const trendOptions: { key: TrendMetric; label: string; unit: string; color: string; icon: keyof typeof MaterialIcons.glyphMap }[] = [
  { key: "heartRate", label: "Heart rate", unit: "BPM", color: "#e91e63", icon: "favorite" },
  { key: "spo2", label: "SpO2", unit: "%", color: "#237d78", icon: "bloodtype" },
  { key: "bodyTemp", label: "Temperature", unit: "C", color: "#e68a2e", icon: "thermostat" },
  { key: "steps", label: "Steps", unit: "steps", color: "#3549b7", icon: "directions-walk" },
  { key: "lightLux", label: "Ambient light", unit: "lux", color: "#c49a18", icon: "light-mode" },
];
...
export default function HomeScreen() {
  ...
  const openProfile = () => { if (user) { setDraft({ ...user }); setPhotoUri(undefined); setProfileOpen(true); } };
  const choosePhoto = async () => {
    const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permission.granted) { Alert.alert("Permission needed", "Allow photo access to add a profile photo."); return; }
    const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ["images"], allowsEditing: true, aspect: [1, 1], quality: 0.9 });
    if (!result.canceled) {
      const cropped = await ImageManipulator.manipulateAsync(result.assets[0].uri, [{ crop: { originX: 0, originY: 0, width: result.assets[0].width, height: result.assets[0].height } }, { resize: { width: 800 } }], { compress: 0.75, format: ImageManipulator.SaveFormat.JPEG });
      setPhotoUri(cropped.uri);
    }
  };
  const saveProfile = async () => {
    setLoading(true);
    try { const saved = await updateProfile(draft, photoUri); setUser(saved); setProfileOpen(false); Alert.alert("Profile updated", "Your patient profile is saved to this device and account."); }
  ...
  const selectedTrend = trendOptions.find((option) => option.key === trendMetric) || trendOptions[0];
  const trendPoints = watchHistory.slice(-trendRange).map((packet) => packet[trendMetric]);
  const trendLabels = watchHistory.slice(-trendRange).map((_, index) => `${index + 1}`);
  if (user)
    return (
      <>
      <StatusBar style="dark" backgroundColor="#f5f7fc" translucent={false} hidden={false} />
      ...
      >
        <View style={styles.dashboardHeader}>
          ...
          <View style={styles.headerActions}>
            ...
            >
              {savedImage ? <Image source={{ uri: savedImage }} style={styles.headerAvatar} /> : <MaterialIcons name="account-circle" size={34} color="#3549b7" />}
            </Pressable>
            <Pressable
        ...
        <Modal visible={profileOpen} animationType="slide" transparent onRequestClose={() => setProfileOpen(false)}>
          <View style={styles.modalBackdrop}><ScrollView contentContainerStyle={styles.profileModal}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>My profile</Text><Pressable onPress={() => setProfileOpen(false)}><Text style={styles.link}>Close</Text></Pressable></View>
            <Pressable style={styles.photoSection} onPress={choosePhoto}>
              <View style={styles.photoFrame}>{photoUri || savedImage ? <Image source={{ uri: photoUri || savedImage }} style={styles.profilePhoto} /> : <Text style={styles.profileInitial}>{displayName.charAt(0).toUpperCase() || "G"}</Text>}<View style={styles.photoEditBadge}><MaterialIcons name="photo-camera" size={16} color="#fff" /></View></View>
              <Text style={styles.photoTitle}>Profile photo</Text>
              <Text style={styles.photoHint}>Tap to choose and crop a clear photo</Text>
            </Pressable>
            <ProfileField label="Name" value={String(draft.name || "")} onChangeText={(v) => setDraft({ ...draft, name: v })} />
            <ProfileField label="Email (cannot be changed)" value={displayEmail} editable={false} />
            <ProfileField label="Date of birth" value={String(draft.dateOfBirth || "")} placeholder="YYYY-MM-DD" onChangeText={(v) => setDraft({ ...draft, dateOfBirth: v })} />
            <Text style={styles.modalSection}>Health status</Text>
            <ProfileField label="BMI" value={String(draft.bmi ?? "")} keyboardType="decimal-pad" onChangeText={(v) => setDraft({ ...draft, bmi: Number(v) })} />
            <SelectField label="Age diagnosed range" value={String(draft.static_age_diagnosed_range || "")} options={["0-6yo", "7-12yo", "13-17yo", "18-40yo", "41-60yo", "61+yo"]} open={openSelect === "age"} onOpen={() => setOpenSelect(openSelect === "age" ? null : "age")} onSelect={(v) => { setDraft({ ...draft, static_age_diagnosed_range: v }); setOpenSelect(null); }} />
            <SelectField label="Severity" value={String(draft.static_severity || "")} options={["Mild", "Moderate", "Severe"]} open={openSelect === "severity"} onOpen={() => setOpenSelect(openSelect === "severity" ? null : "severity")} onSelect={(v) => { setDraft({ ...draft, static_severity: v }); setOpenSelect(null); }} />
            <ProfileField label="Max PEF expected" value={String(draft.static_max_pef_expected ?? "")} keyboardType="numeric" onChangeText={(v) => setDraft({ ...draft, static_max_pef_expected: Number(v) })} />
            <ProfileField label="Pack years" value={String(draft.static_pack_years ?? "")} keyboardType="numeric" onChangeText={(v) => setDraft({ ...draft, static_pack_years: Number(v) })} />
            <ProfileField label="Best PEF" value={String(draft.static_pef_best ?? "")} keyboardType="numeric" onChangeText={(v) => setDraft({ ...draft, static_pef_best: Number(v) })} />
            <Pressable style={styles.submit} onPress={saveProfile} disabled={loading}><Text style={styles.submitText}>{loading ? "Saving..." : "Save profile"}</Text></Pressable>
          </ScrollView></View>
        </Modal>

        <Text style={styles.sectionTitle}>Your health overview</Text>
        <View style={styles.metricsRow}>
          <MetricCard icon="air" label="Air quality" value="Good" tone="teal" />
          <MetricCard
            icon="monitor-heart"
            label="Monitoring"
            value="Ready"
        ...
        <Modal visible={deviceOpen} animationType="slide" transparent onRequestClose={() => setDeviceOpen(false)}>
  ...
  return (
    <>
    <StatusBar style="light" backgroundColor="#253b99" translucent={false} hidden={false} />
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.switchRow}>
          <Text style={styles.switchText}>
            {mode === "login" ? "New to GeneAir?" : "Already have an account?"}
          </Text>
          <Pressable
...
function ProfileField({ label, ...props }: { label: string; [key: string]: any }) {
  return <View style={styles.field}><Text style={styles.label}>{label}</Text><TextInput style={styles.input} placeholderTextColor="#98a3bd" {...props} /></View>;
}
function SelectField({ label, value, options, open, onOpen, onSelect }: { label: string; value: string; options: string[]; open: boolean; onOpen: () => void; onSelect: (value: string) => void }) {
  return <View style={styles.field}><Text style={styles.label}>{label}</Text><Pressable style={styles.input} onPress={onOpen}><Text style={{ color: value ? "#172752" : "#98a3bd" }}>{value || "Select"}</Text></Pressable>{open && <View style={styles.selectMenu}>{options.map((option) => <Pressable key={option} style={styles.selectOption} onPress={() => onSelect(option)}><Text style={styles.selectOptionText}>{option}</Text></Pressable>)}</View>}</View>;
}
...
function MetricCard({
  ...
  return (
    <View style={styles.metricCard}>
      <View
        style={[styles.metricIcon, tone === "teal" ? styles.tealIcon : styles.blueIcon]}
      >
        <MaterialIcons
          name={icon}
          size={21}
...
const styles = StyleSheet.create({
  ...
  headerAvatar: { borderColor: "#cbd3ef", borderRadius: 19, borderWidth: 2, height: 38, width: 38 },
  profileBanner: {
    alignItems: "center",
    backgroundColor: "#fff",
    borderColor: "#e4e8f2",
  ...
  metricValue: { color: "#172752", fontSize: 19, fontWeight: "700", marginTop: 5 },
  insightCard: {
    alignItems: "center",
  ...
  cardEyebrow: { color: "#237d78", fontSize: 10, fontWeight: "700", letterSpacing: 1 },
  insightTitle: { color: "#164d4a", fontSize: 15, fontWeight: "700", marginTop: 4 },
  ...
  actionLabel: { color: "#334064", fontSize: 12, fontWeight: "600", marginTop: 8 },
  welcome: { color: "#172752", fontSize: 26, fontWeight: "700", marginTop: 12, flexShrink: 1 },
  modalBackdrop: { backgroundColor: "rgba(12, 22, 58, 0.42)", flex: 1, justifyContent: "flex-end" },
  profileModal: { backgroundColor: "#f5f7fc", borderTopLeftRadius: 26, borderTopRightRadius: 26, padding: 22, paddingBottom: 40 },
  modalHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 20 },
  modalTitle: { color: "#172752", fontSize: 26, fontWeight: "700" },
  photoSection: { alignItems: "center", backgroundColor: "#e8ebff", borderRadius: 18, marginBottom: 20, padding: 20 },
  photoFrame: { alignItems: "center", backgroundColor: "#dfe5ff", borderColor: "#fff", borderRadius: 52, borderWidth: 4, height: 104, justifyContent: "center", position: "relative", width: 104 },
  profilePhoto: { borderRadius: 48, height: 96, width: 96 },
  profileInitial: { color: "#3549b7", fontSize: 38, fontWeight: "700" },
  photoEditBadge: { alignItems: "center", backgroundColor: "#3549b7", borderColor: "#fff", borderRadius: 15, borderWidth: 2, bottom: -2, height: 30, justifyContent: "center", position: "absolute", right: -2, width: 30 },
  photoTitle: { color: "#172752", fontSize: 15, fontWeight: "700", marginTop: 12 },
  photoHint: { color: "#73809d", fontSize: 12, marginTop: 4 },
  modalSection: { color: "#172752", fontSize: 18, fontWeight: "700", marginBottom: 14, marginTop: 10 },
  selectMenu: { backgroundColor: "#fff", borderColor: "#dce2ef", borderRadius: 9, borderWidth: 1, marginTop: -8, overflow: "hidden" },
  selectOption: { borderBottomColor: "#edf0f7", borderBottomWidth: 1, padding: 13 },
  selectOptionText: { color: "#172752", fontSize: 14 },
  deviceCard: { alignItems: "center", backgroundColor: "#e8ebff", borderRadius: 18, flexDirection: "row", marginBottom: 26, padding: 16 },
  deviceIcon: { alignItems: "center", backgroundColor: "#fff", borderRadius: 12, height: 44, justifyContent: "center", width: 44 },
  ...
  deviceModal: { backgroundColor: "#f5f7fc", borderTopLeftRadius: 26, borderTopRightRadius: 26, padding: 22, paddingBottom: 36 },
  modalCopy: { color: "#73809d", fontSize: 14, lineHeight: 20, marginBottom: 20 },
  orText: { color: "#98a3bd", fontSize: 13, marginVertical: 14, textAlign: "center" },
  scanButton: { alignItems: "center", borderColor: "#cbd3ef", borderRadius: 9, borderWidth: 1, flexDirection: "row", gap: 8, justifyContent: "center", paddingVertical: 14 },
  scanText: { color: "#3549b7", fontSize: 14, fontWeight: "700" },
  scanner: { backgroundColor: "#10152b", flex: 1, justifyContent: "flex-end" },
  camera: { ...StyleSheet.absoluteFillObject },
  scannerClose: { alignItems: "center", backgroundColor: "#3549b7", borderRadius: 9, margin: 24, paddingVertical: 15 },
});
